...
void LoRa::check_lora(struct TGPS *GPS) {
	// CheckFSKBuffer();
	...
	if (LoRaIsFree(GPS))
	{
		...
		if (SendRepeatedPacket == 3)
		{
			...
			Sentence[0] = '%';
			send_lora_packet(Sentence, strlen((char *)Sentence) + 1, 0);
		...
		else if (SendRepeatedPacket == 2)
		{
			...
			send_lora_packet((unsigned char *)&PacketToRepeat, sizeof(PacketToRepeat), 0);
		...
		else
		{
			...
			else
			{
				if ((LORA_CALL_COUNT > 0) && (++CallingCount > LORA_CALL_COUNT))
				{
					...
					send_lora_packet(Sentence, PacketLength, 1);
				}
				else
				{
					...
					send_lora_packet(Sentence, PacketLength, 0);
				}
			}
		}
	}
...
void LoRa::send_lora_packet(unsigned char *buffer, int Length, int CallingPacket) {
	uint8_t buf[1];
	...
	*/
	...
	LoRaMode = lmSending;
	SendingRTTY = 0;
	printf("sending from send_lora_packet...\n");
} // send_lora_packet
